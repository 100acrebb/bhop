...
function Core:LoadZones()
	local qry = "SELECT nType, vPos1, vPos2 FROM game_zones WHERE szMap = '" .. game.GetMap() .. "'"
	local zones = sql.Query( qry )
	...
	SQL:Prepare(
		...
		if data then
			...
			for id,item in pairs( data ) do
				...
				table.insert( Zones.Cache, {
					Type = tonumber( item[ "nType" ] ),
					P1 = util.StringToType( tostring( item[ "vPos1" ] ), "Vector" ),
					P2 = util.StringToType( tostring( item[ "vPos2" ] ), "Vector" )
				} )
...
local SQLDetails = {
	...
	User = "flow_gmod", Pass = "cYdsVxB3cA9vD6aA", -- This is default for a lot of servers, but you'll probably have to change this
	Database = "flow_gmod"
