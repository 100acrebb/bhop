...
function Command:Init()
	...
	self:Register( { "tp", "tpto", "goto", "teleport", "tele" }, function( ply, args )
		...
		if #args > 0 then
			...
			Core:Send( ply, "Print", { "General", "No player name entered. Usage: !tp PlayerName" } )
		end
	end )
	...
	self:Register( { "practice", "try", "free"}, function( ply )
		Command:RemoveLimit( ply )
...
function Command:Trigger( ply, szCommand, szText )
	...
	if not szFunc then return szText end
